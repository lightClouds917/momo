...
public abstract class AbstractPlatformTransactionManager_ implements PlatformTransactionManager, Serializable {
	...
	private static final Constants constants = new Constants(AbstractPlatformTransactionManager_.class);
	...
	public final TransactionStatus getTransaction(TransactionDefinition definition) throws TransactionException {
		...
		if (definition == null) {
			...
			//如果传入的事务定义对象为空，就创建默认的事务定义
			definition = new DefaultTransactionDefinition();
		}

		//检查当前事务是否是一个已经存在的事务，此方法交给具体的事务管理器去实现
		//如果是一个已经存在的事务，会根据事务的定义信息，根据事务的传播属性，对事务作不同的处理，然后返回一个TransactionStatus，此方法后面会具体分析
		if (isExistingTransaction(transaction)) {
			// Existing transaction found -> check propagation behavior to find out how to behave.
			return handleExistingTransaction(definition, transaction, debugEnabled);
		}

		//走到这里，说明当前事务不是一个已经存在的事务，是个新事务
		...
		//对于一个新事务，会根据此事务定义信息的传播属性，来作出不同的处理
		// No existing transaction found -> check propagation behavior to find out how to proceed.
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_MANDATORY) {
			//PROPAGATION_MANDATORY当前有事务，就加入事务，没有就抛出异常
			throw new IllegalTransactionStateException(
					"No existing transaction found for transaction marked with propagation 'mandatory'");
		}

		//PROPAGATION_REQUIRED当前有事务，就加入这个事务，没有事务，就新建一个事务
		//PROPAGATION_REQUIRES_NEW新建一个事务执行，如果当前有事务，就把当前的事务挂起
		//PROPAGATION_NESTED当前有事务，就新建一个事务，嵌套执行，当前无事务，就新建一个事务执行
		else if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_REQUIRED ||
				definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_REQUIRES_NEW ||
				definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NESTED) {
			//挂起指定事务，返回持有挂起资源的对象，后面使用此对象来恢复，见此方法的详细分析
			...
			try {
				...
				//由于这几种传播属性，都是需要新建事务的，那就新建一个DefaultTransactionStatus，这个对象中是持有事务对象的
	...
	private TransactionStatus handleExistingTransaction(
		...
		//PROPAGATION_NEVER在无事务状态下执行，如果当前有事务，会抛出异常
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NEVER) {
			throw new IllegalTransactionStateException(
					"Existing transaction found for transaction marked with propagation 'never'");
		}

		//PROPAGATION_NOT_SUPPORTED在无事务状态下执行，如果当前有事务，就把当前的事务挂起
		...
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_REQUIRES_NEW) {
			...
			//doBegin出错，如果内部事务开始出错，恢复外部事务，且恢复外部事务同步
	...
	/**
	 * 暂停指定的事务，首先暂停事务同步，然后委托给doSuspend模板方法
